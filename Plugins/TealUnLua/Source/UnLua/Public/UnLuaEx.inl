...
namespace UnLua
{
	...
	template <> struct TTypeIntelliSense<void*> { static FString GetName() { return TEXT("any"); } };
	template <> struct TTypeIntelliSense<void> { static FString GetName() { return TEXT(""); } };
	template <> struct TTypeIntelliSense<int8> { static FString GetName() { return TEXT("number"); } };
	template <> struct TTypeIntelliSense<int16> { static FString GetName() { return TEXT("number"); } };
	template <> struct TTypeIntelliSense<int32> { static FString GetName() { return TEXT("number"); } };
	template <> struct TTypeIntelliSense<int64> { static FString GetName() { return TEXT("number"); } };
	template <> struct TTypeIntelliSense<long> { static FString GetName() { return TEXT("number"); } };
	template <> struct TTypeIntelliSense<uint8> { static FString GetName() { return TEXT("number"); } };
	template <> struct TTypeIntelliSense<uint16> { static FString GetName() { return TEXT("number"); } };
	template <> struct TTypeIntelliSense<uint32> { static FString GetName() { return TEXT("number"); } };
	template <> struct TTypeIntelliSense<uint64> { static FString GetName() { return TEXT("number"); } };
	template <> struct TTypeIntelliSense<unsigned long> { static FString GetName() { return TEXT("number"); } };
	...
	struct TArgumentIntelliSense<T1, T2...>
	{
		static void Generate(FString& Buffer, int32 Index)
		{
			...
			// *TArgumentComment<T1>::Get()
			Buffer += FString::Printf(TEXT("arg%d: %s, "), Index, *TypeName);
			TArgumentIntelliSense<T2...>::Generate(Buffer, Index + 1);
		}
	};

	template <typename T1>
	struct TArgumentIntelliSense<T1>
	{
		static void Generate(FString& Buffer, int32 Index)
		{
			FString TypeName = TTypeIntelliSense<typename TChooseClass<TIsPointer<T1>::Value, typename TDecay<typename TRemovePointer<T1>::Type>::Type*, typename TDecay<T1>::Type>::Result>::GetName();
	...
	void GenerateArgsIntelliSense(FString& ArgList)
	{
		// parameters
		TArgumentIntelliSense<ArgType...>::Generate(ArgList, 0);
	}
#endif
	...
	*/
	...
	void TExportedFunction<RetType, ArgType...>::GenerateIntelliSense(FString& Buffer) const
	{
		// arguments
		FString ArgList;
		GenerateArgsIntelliSense<ArgType...>(ArgList);
		...
		// function definition
		Buffer += FString::Printf(TEXT("function(%s) : %s \r\n"), *ArgList, *ReturnTypeName);
	}
#endif
	...
	void TExportedMemberFunction<ClassType, RetType, ArgType...>::GenerateIntelliSense(FString& Buffer) const
	{
		...
		if (ArgList.IsEmpty())
		{
			Buffer += FString::Printf(TEXT("    %s: function(self:%s)%s \r\n"), *Name, *ClassName, *ReturnTypeName);
		}
		else
		{
			Buffer += FString::Printf(TEXT("    %s: function(self:%s, %s)%s \r\n"), *Name, *ClassName, *ArgList, *ReturnTypeName);
		}
	...
	void TExportedStaticMemberFunction<RetType, ArgType...>::GenerateIntelliSense(FString& Buffer) const
	{
		...
		// function definition
		Buffer += FString::Printf(TEXT("	%s: function(%s)%s \r\n"), *Name, *ArgList, *ReturnTypeName);
	}
#endif
	...
	void TExportedClassBase<bIsReflected>::GenerateIntelliSenseInternal(FString& Buffer, FFalse NotReflected) const
	{
		...
		// definition
		Buffer += FString::Printf(TEXT("global record %s\r\n"), *Name);
